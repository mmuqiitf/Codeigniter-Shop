...
class Myorder extends MY_Controller
{
    ...
    public function cancel($invoice)
    {
        ...
        $update = $this->myorder->where('invoice', $data['order']->invoice)->update(['status' => 'cancel']);
        if ($update) {
            $this->session->set_flashdata('success', 'Order berhasil dicancel');
        } else {
            $this->session->set_flashdata('success', 'Terjadi suatu kesalahan');
        }
        redirect(base_url('myorder/detail/' . $invoice));
    }
